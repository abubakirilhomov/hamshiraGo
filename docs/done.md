...
## 2026-04-14 (web: clinic profile page — full UI/UX redesign)

- **[feat]** `web/app/clinics/[id]/page.tsx` — полный редизайн страницы профиля клиники без изменения API/логики. (1) Skeleton-загрузка: hero-блок 200px с teal-shimmer, два stat-скелетона рядом, один card-скелетон с тремя строками докторов — анимация `shimmer` keyframe. (2) Интерфейс `ClinicDoctor` дополнен `specialization?: string | null`; каждая карточка доктора: avatar 52px с цветным кругом, имя 15px bold, специализация 13px gray (или "Специализация не указана" в `#cbd5e1` если отсутствует), тонкий разделитель `border-bottom: 1px solid #f1f5f9` между позициями (не на последней). (3) Услуги: колонка с цветным pill-бейджем категории (`CATEGORY_COLOR` map), название 14px bold с `text-overflow: ellipsis`, длительность 12px gray, цена right-aligned `toLocaleString("ru-RU") + " сум"`, разделители между строками. (4) Stats-карточки: иконка в круге 40px (teal/purple), число 28px 800 weight, подпись 12px, minHeight 90px. (5) Floating CTA заменён на `<a href="tel:...">` «Позвонить в клинику» — height 52px, borderRadius 14px, teal gradient, box-shadow, safe-area-inset padding. `React` добавлен в импорт для `React.CSSProperties`.

## 2026-04-14 (web: patient page — full UI/UX redesign)

- **[feat]** `web/app/patient/page.tsx` — полный редизайн страницы пациента (без изменения API-логики). Pill-таббар с иконками (Clock/FileText/Heart) высотой 44px, горизонтальный scroll без скроллбара, активный таб — сплошной цвет + box-shadow. Skeleton-загрузка с shimmer-анимацией (аватар-круг + строки + 3 pill-таба + 3 карточки). Карточки визитов: цветная полоса-акцент 4px слева, статус-бейдж pill UPPERCASE, дата в русской локали (formatDateRu), pill-тег услуги (teal). Карточки рецептов: purple-gradient фон (`#faf5ff→#f5f3ff`), border `#e9d5ff`, номер рецепта из последних 8 символов UUID, preview 60 символов, hover border/shadow, кнопка «Открыть →». Медкарта: placeholder с 72px кругом (red-50), заголовок + описание, 3 teal info-chips (Анамнез / Аллергии / Диагнозы). Кнопка «Скачать историю PDF» — ghost-стиль (teal border), иконка Download, показывается только при `visits.length > 0 && tab === "visits"`. CSS-классы `.tab-bar`, `.rx-card:hover`, `.visit-card:hover` через inline `<style>`. Все touch-targets ≥ 44px.

## 2026-04-14 (web: clinics list page — full UI/UX redesign)

- **[feat]** `web/app/clinics/page.tsx` — полный редизайн страницы списка клиник без изменения логики. Hero-секция с градиентом `#0d9488→#0f766e`, stat-бейдж «N клиник доступно», заголовок + подзаголовок. Поисковая строка «плавает» с отрицательным margin-top -24px, border анимируется в teal при фокусе, кнопка × для очистки. Карточки переведены с flex-списка на CSS-grid (`repeat(auto-fill, minmax(280px, 1fr))`). Каждая карточка: avatar (gradient bg + первая буква) / logo image, name + city badge, address (MapPin), phone (tel: link, Phone icon), specialization pills (до 3 + «+N»), chip с кол-вом врачей (Users icon), «Подробнее →». Hover: border teal, box-shadow teal, translateY(-2px), 200ms. Skeleton-загрузка — 4 карточки с shimmer-анимацией. Empty-state с иконкой Building2, кнопкой «Сбросить поиск». Error-state с красным Building2 и кнопкой «Повторить». Добавлены иконки Phone, Users, X (lucide-react). Все touch-targets ≥ 44px. maxWidth 720px.

## 2026-04-14 (web: clinic profile — category filter tabs for services)

- **[feat]** `web/app/clinics/[id]/page.tsx` — добавлены фильтр-табы по категории услуг. `useMemo` добавлен в импорт. Состояние `activeCategory` (по умолчанию "ALL"). Константа `CATEGORY_LABELS` с переводами (`CONSULTATION`, `LAB`, `DIAGNOSTIC`, `PROCEDURE`). `availableCategories` вычисляется из реальных данных (уникальные категории + "ALL"), табы рендерятся только если категорий > 2. `filteredServices` фильтрует список по активной категории. При пустом результате показывается "Нет услуг в этой категории". `clinic.services.map` заменён на `filteredServices.map`.

## 2026-04-14 (Doctor: prescription print page — replace alert stub)

- **[fix]** `web-medic/app/doctor/consultation/[id]/page.tsx` — `handleSendPrescription` больше не показывает alert-заглушку. Теперь сериализует `prescriptionItems` + данные врача/пациента в `sessionStorage("print_prescription")` и открывает `/doctor/prescription/print` в новой вкладке. Кнопка переименована: «Отправить рецепт пациенту» → «Сформировать рецепт (PDF)». `handleScheduleNextVisit` показывает «Функция будет доступна в ближайшее время» вместо старого stub-текста.
- **[feat]** `web-medic/app/doctor/prescription/print/page.tsx` — новая клиентская страница (no-layout). Читает `sessionStorage("print_prescription")`, рендерит A4-документ: шапка HamshiraGo с датой, блок врача и пациента, таблица назначений (препарат / доза / кратность / дней), строка подписи врача. Автовызов `window.print()` через 600ms. Кнопки «Печать / Сохранить PDF» и «Закрыть» скрыты в `@media print`. Если sessionStorage пуст — экран ошибки с кнопкой «Закрыть».

## 2026-04-14 (Patient: prescription detail page — real API + print page)

- **[fix]** `web/app/patient/prescriptions/[id]/page.tsx` — удалён MOCK_PRESCRIPTIONS и alert-заглушка PDF. Страница теперь грузит реальные данные из `GET /patient/prescriptions/:id` (Bearer-токен из localStorage). Добавлены loading/error-состояния, 401-редирект на `/auth` с очисткой токена, 404-редирект на back. Поле `content` рендерится как `<pre>` (preformatted-блок) вместо структурированного массива drugs. Поле `notes` показывается в зелёной карточке «Рекомендации врача». Бейдж статуса (`ACTIVE`/`COMPLETED`/`CANCELLED`) с цветовой кодировкой. Дата в русской локали. Кнопка «Распечатать / Скачать PDF» открывает `/patient/prescriptions/:id/print` в новой вкладке.
- **[feat]** `web/app/patient/prescriptions/[id]/print/page.tsx` — новая клиентская страница (создана директория `[id]/print/`). Паттерн идентичен `visits/print/page.tsx`: грузит `GET /patient/prescriptions/:id` по токену (редирект на `/auth` без токена / 401), автовызов `window.print()` через 500ms. A4-документ: шапка «HamshiraGo — Медицинский рецепт», пациент + телефон + дата выдачи + номер рецепта, блок назначений (`content` preformatted), блок рекомендаций врача (при наличии `notes`), футер со статусом и датой формирования. Кнопки «Печать» и «Закрыть» скрыты в `@media print`. Inline-стили, без новых npm-зависимостей.

## 2026-04-12 (Жафар — Patient: экспорт истории визитов в PDF)

- **[feat]** `web/app/patient/page.tsx` — добавлена кнопка «Скачать PDF» над списком визитов (видна только если `visits.length > 0`), открывает `/patient/visits/print` в новой вкладке.
- **[feat]** `web/app/patient/visits/print/page.tsx` — новая клиентская страница: грузит `/auth/me` и `/patient/visits` по токену из localStorage (редирект на `/auth` если токена нет / 401), рендерит A4-документ (`@page { size: A4; margin: 20mm }`, `max-width: 210mm`) с шапкой «HamshiraGo — История визитов», ФИО и телефоном пациента, таблицей визитов (№, дата, время, клиника, врач, услуга, статус) и футером (всего визитов, дата формирования). Автовызов `window.print()` через `setTimeout(500ms)` после загрузки. Кнопки «Печать» и «Закрыть» скрыты в `@media print`. Без новых npm-зависимостей (`window.print()` → системный PDF-экспорт браузера).

## 2026-04-12 (Жафар — Reception: переключатель Список/Календарь)

- **[feat]** `web-medic/app/clinic/reception/page.tsx` — добавлен тогглер «Список / Календарь» в шапке (активная кнопка — teal-gradient). По умолчанию отображается прежний списочный вид (не изменён). При выборе «Календарь» рендерится дневной view: левая ось времени 08:00–20:00 с шагом 30 мин, колонки — кабинеты (`clinicApi.rooms.list()`), при отсутствии комнат — врачи (`clinicApi.stats.doctors()`), иначе одна колонка «Все». Визиты отображаются карточкой в ячейке `(time, room/doctor)` с цветом по статусу (teal/amber/blue/green/red/slate), клик — модалка деталей (пациент, телефон, кабинет, врач, оплата, статус). Навигация по датам: кнопки `← / Сегодня / →`, дата прописью в центре, `Обновить`. Загрузка через `clinicApi.appointments.list({ date })`. Responsive: `overflow-x: auto` на узких экранах. drag&drop не реализован (V2). Без новых npm-зависимостей.

## 2026-04-12 (Жафар — CEO Dashboard: новые KPI-виджеты)

- **[feat]** `web-medic/app/clinic/dashboard/page.tsx` — добавлены два виджета в стиле существующих (teal/slate, inline-styles, skeleton/error-состояния). Агрегация на клиенте через существующие эндпоинты бэкенда (новые не добавлялись). 1) **«Загрузка кабинетов за неделю»** — `clinicApi.rooms.list()` + `clinicApi.appointments.list({ date })` за последние 7 дней; % = non-canceled визитов / (7×8 слотов), прогресс-бар с цветом по порогу (≥80% красный, ≥50% teal, иначе серый). 2) **«Топ врачей по визитам»** — `clinicApi.staff.list()` (filter role=DOCTOR) + `appointments.list({ date, status: "DONE" })` за 30 дней, top-5 с нумерацией и аватаром. Оба виджета в новой grid-строке (`clinic-grid-stack`) — не ломают существующий layout. Без новых npm-зависимостей.

## 2026-04-12 (Жафар — PDF-экспорт заключения консультации)

- **[feat]** Doctor portal: экспорт заключения консультации в PDF через `window.print()`. Создана страница `web-medic/app/doctor/consultation/[id]/print/page.tsx` — A4-совместимый ч/б документ (шапка «HamshiraGo — Медицинское заключение», пациент, дата, симптомы, Salomat AI, заключение врача, назначение, подпись). На `useEffect` автоматически открывает диалог печати через `setTimeout(..., 500)`. В `web-medic/app/doctor/consultation/[id]/page.tsx` добавлена кнопка «Скачать заключение (PDF)» в правой колонке (иконка `Download`), видна только при `status === 'COMPLETED' && doctorNotes`. Без новых npm-зависимостей.

## 2026-04-12 (Жафар — CLIN-FE-12/10/9 — реальные API)

- **[CLIN-FE-12-API]** Публичный API клиник — добавлены методы `listPublicClinics(q?)` и `getPublicClinicDetail(id)` в `backend/src/clinic/clinic.service.ts`; эндпоинты `GET /companies` и `GET /companies/:id` в `ClinicPublicController` (`backend/src/clinic/clinic.controller.ts`). Возвращают только `isActive=true AND isVerified=true` клиники, со списком врачей (id/name) и услуг (id/name/category/price/durationMinutes). Specializations = distinct service categories. Список — с поиском по `?q=` (name/address/city). Frontend: `web/app/clinics/page.tsx` и `web/app/clinics/[id]/page.tsx` переписаны, mock удалён, добавлены loading/error states.
- **[CLIN-FE-10-API]** История визитов пациента — добавлен сервис-метод `getMyVisits(patientId)` (join-ит `companies`, `company_users`, `company_services`) и эндпоинт `GET /patient/visits` в `PatientPrescriptionsController` (JwtAuthGuard). Frontend `web/app/patient/page.tsx` — теперь грузит `/auth/me`, `/patient/visits`, `/patient/prescriptions`, корректные статусы визитов со стилями, редирект на `/auth` при 401, empty states.
- **[CLIN-FE-9-ROOM]** BookingModal: привязка кабинета к расписанию врача. Backend: `getRoomsForDoctor(companyId, doctorId, date)` + `GET /clinic/doctors/:doctorId/rooms?date=YYYY-MM-DD`. Web-medic: `clinicApi.rooms.forDoctor(doctorId, date)`, новый тип `DoctorRoomSlot`. В `BookingModal.tsx`: после выбора врача+даты подгружается список кабинетов с окнами работы, селектор кабинета обязателен, валидация времени в пределах startTime–endTime слота. Убран fallback `roomId = doctorId`.

## 2026-04-12 (Жафар — триаж Clinic bugs)

- **[verify]** CLIN-BUG-1 — подтверждено: `web-medic/app/clinic/layout.tsx:121-163` корректно обрабатывает `isAuthPage` (инициализирует role=GUEST, early return из useEffect, рендерит children без sidebar). Запись удалена из `docs/tasks.md`.
- **[verify]** CLIN-BUG-2 — подтверждено: `web-medic/app/clinic/dashboard/page.tsx:224, 258` выручка форматируется как `сум` (UZS), не ₽. Запись удалена из `docs/tasks.md`.
