...
> Sana: 2026-03-01 (oxirgi yangilanish: 2026-03-02)
> Jami: 39 ta bug topildi (7 kritik, 8 yuqori, 18 o'rta, 6 past)
> Holat: ✅ 16 ta tuzatildi | ❌ 13 ta ochiq (backend/mobile/medic) | ⛔ 10 ta read-only (web/admin)
...
### BUG 3 ✅ ИСПРАВЛЕНО (Этап 1): WebSocket avtorizatsiyasiz
...
### BUG 4 ✅ ИСПРАВЛЕНО (Этап 1): GET /orders/:id egasi tekshiruvisiz
...
### BUG 5 ✅ ИСПРАВЛЕНО (Этап 3): Admin parol timing-attack ga zaif
...
### BUG 6 ✅ ИСПРАВЛЕНО (Этап 3): Telefon raqam validatsiyasi yo'q
...
### BUG 7 ✅ ИСПРАВЛЕНО (Этап 3): RegisterClientDto.name ixtiyoriy maydon validatsiyasi
...
### BUG 8 ✅ ИСПРАВЛЕНО (Этап 3): Medik refreshProfile noto'g'ri pattern
...
### BUG 9 ✅ ИСПРАВЛЕНО (Этап 1): ClientId dekoratori bo'sh string qaytaradi
...
### BUG 10 ✅ ИСПРАВЛЕНО (Этап 1): updateStatus holatlar mashinasi validatsiyasiz
...
### BUG 11 ✅ ИСПРАВЛЕНО (Этап 1): WebSocket CORS barcha originlarga ruxsat beradi
...
### BUG 12 ✅ ИСПРАВЛЕНО (Этап 3): Admin login body validatsiyasi yo'q
...
### BUG 13 ✅ ИСПРАВЛЕНО (Этап 1): Route mos kelish konflikti
...
### BUG 14 ⛔ READ-ONLY: web-medic barcha buyurtmalarni yuklaydi bitta topish uchun
...
### BUG 15 ⛔ READ-ONLY: Web client CreateOrderDto ortiqcha maydonlar
...
### BUG 16 ❌ ОТКРЫТ: Web push eskirgan obunalarni o'chirish
...
### BUG 17 ❌ ОТКРЫТ: notifyClient push operatsiyalarini kutmaydi
...
### BUG 18 ❌ ОТКРЫТ: JwtPayload turida admin roli yo'q
...
### BUG 19 ❌ ОТКРЫТ: discountAmount entity tur nomuvofiqligi
...
### BUG 20 ✅ ИСПРАВЛЕНО (Этап 3): Auth token saqlanmaydi (mobil ilovalar)
...
### BUG 21 ❌ ОТКРЫТ: Mavjud bo'lmagan routega navigatsiya
...
### BUG 24 ❌ ОТКРЫТ: Bosh sahifa xatolarni jimgina yutadi
...
### BUG 25 ❌ ОТКРЫТ: Medik buyurtmalar sahifasi xatolarni e'tiborsiz qoldiradi
- **Fayl:** `medic/app/(tabs)/index.tsx:122-124`
- **Muammo:** `fetchOrders` barcha xatolarni ushlaydi va hech narsa qilmaydi.

### BUG 26 ⛔ READ-ONLY: Qattiq kodlangan localhost:3000
...
### BUG 27 ⛔ READ-ONLY: API javob turi nomuvofiqligi
...
### BUG 28 ⛔ READ-ONLY: WebSocket tozalash muammosi
...
### BUG 29 ⛔ READ-ONLY: Web client to'g'ridan-to'g'ri DONE statusiga o'tkazishi mumkin
...
### BUG 30 ⛔ READ-ONLY: Dashboard daromad faqat birinchi 100 ta buyurtmani hisobga oladi
...
### BUG 31 ⛔ READ-ONLY: Dashboard "bugun" hisob 100 ta bilan cheklangan
...
### BUG 32 ⛔ READ-ONLY: Admin routelar faqat token mavjudligini tekshiradi
...
### BUG 33 ⛔ READ-ONLY: Admin JWT localStorage da — XSS ga zaif
...
### BUG 34 ✅ ИСПРАВЛЕНО (Этап 3, = BUG 7): name maydonda @IsOptional() yo'q
- **Fayl:** `backend/src/auth/dto/register-client.dto.ts`

### BUG 35 ❌ ОТКРЫТ: User.phone da DB unique constraint yo'q
...
### BUG 36 ❌ ОТКРЫТ: Order.location aloqasi mo'rt
...
### BUG 37 ❌ ОТКРЫТ: Medik balance tur nomuvofiqligi
...
### BUG 38 ✅ ИСПРАВЛЕНО косвенно (Этап 1, через BUG 3): medic_location event GPS ni barcha obunachilarga tarqatadi
...
### BUG 39 ✅ ИСПРАВЛЕНО (Этап 1): handleMedicLocation medik tayinlanganligini tekshirmaydi
...
## Eng shoshilinch 5 ta tuzatish (yangilangan 2026-03-02)

> ~~BUG 2+10, 13, 11, 20~~ — hammasi tuzatildi (Этап 1 va Этап 3)

**Hozirda eng muhim ochiq buglar:**

1. **BUG 1** ❌: Race condition — atomik `UPDATE ... WHERE status='CREATED'` (`backend/src/orders/orders.service.ts`)
2. **BUG 21** ❌: Mavjud bo'lmagan `/order/track` routega navigatsiya (`mobile/app/(tabs)/two.tsx`)
3. **BUG 17** ❌: `notifyClient` push-larni `await` qilmaydi (`backend/src/orders/orders.service.ts`)
4. **BUG 18** ❌: `JwtPayload` turida `admin` roli yo'q (`backend/src/auth/strategies/jwt.strategy.ts`)
5. **BUG 24+25** ❌: Xatolar jimgina yutiladi, foydalanuvchi bo'sh ekran ko'radi (`mobile`, `medic`)
